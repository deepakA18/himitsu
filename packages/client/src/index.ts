...
export class RpcClient {
  ...
  async request<T>(method: string, params: unknown[] = []): Promise<T> {
    ...
    const body = await response.json() as { id?: unknown; jsonrpc?: unknown; result?: T; error?: { code: number; message: string } };
...
export interface DeploymentIdentity { chainId: bigint; genesisHash: Hex; }
...
export async function verifyDeployment(rpc: RpcClient, expected: DeploymentIdentity) {
  ...
  if (actual.chainId !== expected.chainId || actual.genesisHash.toLowerCase() !== expected.genesisHash.toLowerCase()) {
...
export interface PendingTransaction { hash: Hex; raw: Hex; state: SubmissionState; }
...
export async function submitPrepared(
  ...
  if (transaction.state !== 'prepared') throw new Error('Reconcile existing submission before retrying');
  const { keccak256 } = await import('viem');
  assertHex(transaction.raw); assertHex(transaction.hash, 32);
  ...
  try {
    ...
    if (hash.toLowerCase() !== transaction.hash.toLowerCase()) throw new Error('Unexpected submitted hash');
