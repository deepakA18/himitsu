...
export interface VaultStore {
  ...
  compareAndSet(expected: number | null, value: Envelope): Promise<void>;
}
...
function envelope(value: unknown): Envelope {
  ...
  if (
    !e ||
    e.format !== 'himitsu-vault' ||
    e.version !== 1 ||
    !Number.isSafeInteger(e.revision) ||
    e.revision < 0 ||
    typeof e.ciphertext !== 'string' ||
    e.ciphertext.length > 10_000_000 ||
    decode(e.salt).length !== 16 ||
    decode(e.iv).length !== 12
  )
    throw new Error('Unsupported vault backup');
  decode(e.ciphertext);
  return e;
}
function validate(data: VaultData) {
  ...
  for (const a of data.attempts) {
    ...
    for (const v of [a.nonce, a.deadline])
      if (v !== undefined && !/^\d+$/.test(v)) throw new Error('Invalid attempt number');
    if (
      a.kind !== 'deposit' &&
      (!a.raw || !a.hash || a.nonce === undefined || a.deadline === undefined)
    )
      throw new Error('Incomplete frame attempt');
  }
}
...
async function decrypt(e: Envelope, key: CryptoKey) {
  ...
  try {
    plain = await crypto.subtle.decrypt(
      { name: 'AES-GCM', iv: decode(e.iv), additionalData: aad(e.revision) },
      key,
      decode(e.ciphertext),
    );
  } catch {
...
async function encrypt(
  ...
  const ciphertext = await crypto.subtle.encrypt(
    { name: 'AES-GCM', iv, additionalData: aad(revision) },
    key,
...
export class Vault {
  ...
  static async restore(store: VaultStore, password: string, text: string) {
    ...
    const saved = envelope(JSON.parse(text)),
      key = await derive(password, saved.salt),
      data = await decrypt(saved, key);
    await store.compareAndSet(null, saved);
    return new Vault(store, key, saved, data);
  }
  async reload() {
    const saved = await this.store.read();
    if (!saved || saved.salt !== this.saved.salt)
      throw new Error('Vault changed; lock and unlock again');
    const data = await decrypt(envelope(saved), this.key);
    this.saved = saved;
    this.data = data;
  }
  async save(next: VaultData) {
    const fresh = await encrypt(next, this.key, this.saved.salt, this.saved.revision + 1);
    await this.store.compareAndSet(this.saved.revision, fresh);
    this.saved = fresh;
    this.data = structuredClone(next);
  }
  async backup() {
    await this.reload();
    return JSON.stringify(this.saved, null, 2);
  }
}
  ...
  async read(): Promise<Envelope | undefined> {
    ...
    return new Promise((resolve, reject) => {
      const tx = db.transaction('vault', 'readonly'),
        req = tx.objectStore('vault').get('document');
      tx.oncomplete = () => {
        db.close();
        resolve(req.result);
      };
  ...
  async compareAndSet(expected: number | null, value: Envelope): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const tx = db.transaction('vault', 'readwrite', { durability: 'strict' }),
        ...
        req = store.get('document');
      let conflict = false;
      req.onsuccess = () => {
        const current = req.result as Envelope | undefined;
        if ((current?.revision ?? null) !== expected) {
          conflict = true;
          tx.abort();
        } else store.put(value, 'document');
      };
      tx.oncomplete = () => {
        db.close();
        resolve();
      ...
      tx.onabort = tx.onerror = () => {
        ...
        reject(
          new Error(
            conflict
              ? 'Vault changed in another tab. Refresh before continuing.'
              : 'Could not durably save the vault',
          ),
        );
      };
    });
...
export const isActive = (a: Attempt) =>
  !['confirmed', 'failed', 'conflict', 'expired'].includes(a.state);
