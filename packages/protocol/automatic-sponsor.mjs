...
export async function deployAutomaticSponsor(pools, { funding = '0.1ether' } = {}) {
  const policy = {
    chainId: BigInt(await rpc('eth_chainId')),
    pools: pools.map(address => ({ address, verifySelector: iface.validateSpend, verifyCalldataBytes: 100,
      executeSelector: iface.spend, executeCalldataBytes: 4,
      additionalExecutions: [{ selector: iface.spendAndSwapToNote, calldataBytes: 164 }] })),
